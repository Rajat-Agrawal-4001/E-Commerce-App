...
function update_margin_ebanking($prod_id,$marketplace,$user_type,$vendor_id,$reason_for_transaction = 'NOT SPECIFIED', $attachment = '')
{
    global $conn, $curr_date, $this_site_id;
    if ($vendor_id == -1)
        $vendor_id = getVendorID();

$arr=margin($user_type,$prod_id,$marketplace);

$total_margin=$arr['MARGIN'];

$qry = "INSERT INTO wallet_transactions (transact_from, vendor_id, nature, date, amount, status, description, site_id, attachment) values ('ebanking', '$vendor_id', 'CREDIT', '$curr_date', '$total_margin', 'COMPLETED', '$reason_for_transaction', '$this_site_id', '$attachment') ";
